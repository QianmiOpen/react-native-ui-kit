...
class AdvanceScrollView extends React.Component {
  ...
  render () {
    let RefreshControl = Config.pullRefresh.control,
      {onRefresh} = this.props,
      {pullHeight} = this.state;
    return (
      <ScrollView
        ...
        {onRefresh ? <RefreshControl pullHeight={pullHeight} onRefresh={onRefresh}/> : null}
        {this.props.children}
      </ScrollView>
    );
  }
  ...
  _handleScroll = (e:Object) => {
    ...
    if (offset < 0 && absOffset >= onTopHeight) {
      ...
      if (onRefresh) {
        this.setState({pullHeight: absOffset});
      }
    }
    //往上推
